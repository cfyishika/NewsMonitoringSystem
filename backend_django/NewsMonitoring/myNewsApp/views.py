        ...
        try:
            if password1 == password2:
                ...
                if User.objects.filter(
                    ...
                    client, company = client_data[0], company_data[
                        0]  # get data from list of dropdown
                    queryset = all_companies.filter(
                        Q(company_name=company) | Q(
                            company_name=client))  # get company and client in the form of list
                    if (client == company):
                        registered_client = registered_company = \
                        queryset[0]
                    else:
                    ...
                    Subscriber.objects.create(user=user,
                                              company_data=registered_company,
                                              client=registered_client).save()
                    messages.info(request,
                                  "Success!! registered successfully")
                    return redirect('/')
            else:
                print(
                    "Check your passwords !! Confirm password not matching to password")
                messages.info(request,
                              'Check your passwords !! Confirm password not matching to password')
                return redirect('/register')
        except ValueError:
            messages.info(request,'Enter the required details please!!')
            return redirect('/register')
